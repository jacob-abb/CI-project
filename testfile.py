...
if True:
	...
	class ADO_Workitems_Task():
		...
		def __init__(self, **kwargs) -> None:
			...
			self.organization_url = kwargs["organization"]
			self.task_name = kwargs["task_name"]
			# 你的个人访问令牌（PAT），请确保不要在代码库中直接暴露它  
			# 为了安全起见，最好使用环境变量或加密的存储来管理PAT  
			self.personal_access_token = kwargs["query_id"]
			self.big_ver = kwargs["big_ver"]
			self.sub_ver = kwargs["sub_ver"]
			self.project_name = self.get_project_id(kwargs["project"]) # Azure DevOps项目的名称 "PCP-Test" "PCP"    

			# 打印脚本文件所在的目录  
			print("脚本所在目录是:", self.script_dir)
			print(self.project_name)

		def get_project_id(self, name="PCP"):
			
			# 将PAT添加到HTTP请求的Authorization头中  
			encoded_creds = base64.b64encode(f":{self.personal_access_token}".encode()).decode()  
			...
			# 发送GET请求以获取所有项目  
			projects_url = f"{self.organization_url}/_apis/projects?api-version={self.api_version}"  
		...
		def update_workitems(self, TARGET=["Bug", 'Epic_Feature']):
			...
			area_path = f"PCP\\Operations\\NextGenHMI\\{self.task_name}\\{self.big_ver}\\{self.sub_ver}"
			# 'PCP\\\\Operations\\\\NextGenHMI\\\\Certificate Management\\\\2.0\\\\2.0.0'
			# 'PCP\\\\Operations\\\\NextGenHMI\\\\Operations Client\\\\3.0\\\\3.0.0'
			# 'PCP\\\\Operations\\\\NextGenHMI\\\\Deploy Tool Components\\\\1.0\\\\1.0.0'
			# 'PCP\\\\Operations\\\\NextGenHMI\\\\Configuration Manager\\\\1.0\\\\1.0.0'
			ql_path = f'PCP\\\\Operations\\\\NextGenHMI\\\\{self.task_name}\\\\{self.big_ver}\\\\{self.sub_ver}'

			for DEF_TYPE in TARGET:  
				if DEF_TYPE == "Bug":
					self.useful_info = {"id": "id",
									"rev": "rev",
				...
				if DEF_TYPE == 'Bug':
					...
					ql_str = """
					{
						"query": "SELECT [System.Id], [System.WorkItemType], [System.Title], [System.State], [System.AreaPath] FROM WorkItems WHERE [System.WorkItemType] IN ('Bug') AND [System.AreaPath] UNDER """+ql_path+""" ORDER BY [System.Id] DESC"
					}
					"""
					self.query_workitems(DEF_TYPE, area_path, ql_str)

				elif DEF_TYPE == 'Epic_Feature':
					# 'PCP\\\\Operations\\\\NextGenHMI\\\\Certificate Management\\\\2.0\\\\2.0.0'
					ql_str = """
					{
						"query": "SELECT [System.Id], [System.WorkItemType], [System.Title], [System.State], [System.AreaPath] FROM WorkItems WHERE [System.WorkItemType] IN ('Epic', 'Feature') AND [System.AreaPath] UNDER """+ql_path+""" ORDER BY [System.Id] DESC"
					}
					"""
					self.query_workitems(DEF_TYPE, area_path, ql_str)
				else:
		...
		def query_workitems(self, type, area_path, ql_str):

			wiql_url = f'{self.organization_url}/{self.project_name}/_apis/wit/wiql?api-version=6.0'  
			...
			response = requests.post(wiql_url, headers=headers, json=wiql, auth=HTTPBasicAuth('', self.personal_access_token))  
			...
			if response.status_code == 200:  
				...
				for work_item in work_items:  
					...
					wiql_url = f'{self.organization_url}/{self.project_name}/_apis/wit/workItems/{work_item["url"].split("/")[-1]}'  
					# 创建头部信息  
					encoded_credentials = base64.b64encode(f":{self.personal_access_token}".encode('ascii')).decode('ascii')  
					...
					if response.status_code == 200:  
						...
						flat_dict = self.flatten_dict(work_item["fields"])  
				...
				target_list = list(self.useful_info.keys())
				for col in target_list:
					if col not in df_total.columns:
						self.useful_info.pop(col)
		...
		def compare_feature_epic_state(self, baseline_path="C:\\Users\\CNJAYUA1\\Downloads\\ADO Project\\NGT Engineering 7.0 lifecycle&readiness baseline M2.csv", 
			...
			for key in define_type:
				...
				markdown_content += f"#{key} report:" # 一级标题
		...
		def compare_bugs(self, baseline_path="C:\\Users\\CNJAYUA1\\Downloads\\ADO Project\\Scope bugs 7.0&xP0 M2 Baseline.csv", 
	...
	task_obj = ADO_Workitems_Task(**{"organization":organization, 
								     "project":project, 
									 "query_id":query_id,  
									 "task_name":task_name, 
									 "big_ver":big_ver, 
	...
	task_obj.update_workitems(["Bug", 'Epic_Feature'])
